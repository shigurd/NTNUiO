...
class QuizApp(tk.Tk):
    ...
    def create_normal_overview_container(self):
        ...
        box_label_info_button.menu.add_cascade(label='Det er mulig å lese tidligere besvarte oppgaver, men disse kan ikke besvares på nytt. Oppgaver man hopper over markeres som feil')
        box_label_info_button.menu.add_cascade(label='Statistikk lagres når man trykker "Hjem" etter at quiz er startet')
    ...
    def create_doctor_overview_container(self):
        ...
        box_label_info_button.menu.add_cascade(label='Statistikk lagres når man trykker "Hjem" etter at quiz er startet. Ved prematur avslutning blir gjenværende liv beregnet som feil')
    ...
    def update_statistics_doctor(self):
        ...
        if float(self.statistics_obj.statistics_dict['OVERLEGEMODUS']['Beste score (% riktige)'].split('%', 1)[0]) < round((self.correct_questions * 100 / (self.current_largest_question_index + 1)), 1):
...
if __name__ == '__main__':
    ...
    question_set_json_pths = []
    for f in os.listdir('.'):
        if f.rsplit('.', 1)[-1] == 'json' and f.rsplit('.', 1)[0] != 'statistics':
            question_set_json_pths.append(f)
    #print(question_set_json_pths)
